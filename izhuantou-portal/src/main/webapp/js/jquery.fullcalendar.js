...
//						}
						if (info) {
							var data1 = info.sYear
			                + "-"+ ((info.sMonth ) >= 10 ? (info.sMonth ) : "0"
			                        + (info.sMonth )) + "-" + (info.sDay < 10 ? "0" + info.sDay : info
			                        .sDay);
							var returnSize = ($("#returnSize")).text();
							
							for (var q = 1; q < returnSize; q++) {
                            var returnDate = ($("#returnDate"+q).text());
								if(data1==returnDate ){	
									if(dayHtml.indexOf("还款日") < 0){
										dayHtml+='<span color="#ed0000">还款日</span>';
									}
									
								}
							}
							var repaySize = ($("#repaySize")).text();
							for (var w = 1; w < repaySize; w++) {
	                            var repayDate = ($("#repayDate"+w).text());
									if(data1==repayDate ){	
										if(dayHtml.indexOf("回款日") < 0){
											dayHtml+='<span color="#ed0000">回款日</span>';
							...
							var finishReturnSize = ($("#finishReturnSize")).text();
							for (var w = 1; w < finishReturnSize; w++) {
	                            var repayDate = ($("#finishReturnDate"+w).text());
									if(data1==repayDate ){	
										if(dayHtml.indexOf("回款日") < 0){
											dayHtml+='<span color="#ed0000">已回款日</span>';
											//var data2=info.sYear+ "-"+ info.sMonth + "-" + info.sDay;
											//alert($('td[abbr="2017-8-1"]').text());
											//alert(t.find('td[abbr="2017-8-1"]').text());
											//t.find('td[abbr="' + data2 + '"]').css('backgroundColor', 'rgb(129, 208, 242)');
											//$("#finishReturnDate"+w).par.css("background-color","ffa5a5");
										}
										
									}
								}
							
							var agentReturnSize = ($("#agentReturnSize")).text();
							for (var w = 1; w < agentReturnSize; w++) {
	                            var repayDate = ($("#agentReturnDate"+w).text());
									if(data1==repayDate ){	
										if(dayHtml.indexOf("回款日") < 0){
											dayHtml+='<span color="#ed0000">待回款日</span>';
										}
				...
				var day2 = $('<td class="calendar-day fullcalendar-day calendar-other-month"></td>').data('info', info).attr('abbr', day[0] + '-' + day[1] + '-' + day[2]).html(dayHtml).appendTo(tr);
				if (info && (info.lunarFestival || info.solarFestival)) {
					day2.addClass('festival');
				}
				var day3= $(".due_in,.Received_in");
				/*   ---------------   */
				if (opts.month == day[1]) {
					info = currentCa[day[2] - 1];
					if (info.color) {
...
//						}
						if (info) {
							...
							for (var w = 1; w < finishReturnSize; w++) {
	                            var repayDate = ($("#finishReturnDate"+w).text());
									if(data1==repayDate ){
										//alert(dayHtml);
										if(dayHtml.indexOf("已回款日") > 0){
											var data2=info.sYear+ "-"+ info.sMonth + "-" + info.sDay;
											//alert(data2);
											//alert(t.find('td[abbr="2017-8-1"]').text());
											t.find('td[abbr="' + data2 + '"]').addClass("due_in");
											//$("#finishReturnDate"+w).par.css("background-color","ffa5a5");
										}
							...
							for (var w = 1; w < agentReturnSize; w++) {
	                            var repayDate = ($("#agentReturnDate"+w).text());
									if(data1==repayDate ){	
										if(dayHtml.indexOf("待回款日") > 0){
											var data2=info.sYear+ "-"+ info.sMonth + "-" + info.sDay;
											//alert(data2);
											//alert(t.find('td[abbr="2017-8-1"]').text());
											t.find('td[abbr="' + data2 + '"]').addClass("Received_in");
										}
				...
				day3.hover(function (e) {
					clearTimeout(hideTimer);
					var inf = $(this).data('info');
					
					if (inf) {
						var data1 =  inf.sYear
		                + "-"+ ((inf.sMonth ) >= 10 ? (inf.sMonth ) : "0"
	                        + (inf.sMonth )) + "-" + (inf.sDay < 10 ? "0" + inf.sDay : inf
	                        .sDay);
...
//							 + ' 日</font><br><font color="yellow">' + inf.cYear + '年 ' + inf.cMonth + '月 ' + inf.cDay + '日</font></font>';
						var returnSize = ($("#returnSize")).text();
						for (var q = 1; q < returnSize; q++) {
                            var data2 = ($("#returnDate"+q).text());
								if(data1==data2 ){	
									ct = ct+  '<br> 还款本金:' +$("#principalMoney"+q).text()
									 + '<br> 还款利息:' +$("#interestMoney"+q).text()
								}
							}
						var repaySize = ($("#repaySize")).text();
						for (var w = 1; w < repaySize; w++) {
							  var data2 = ($("#repayDate"+w).text());
								if(data1==data2 ){	
									ct = ct+  '<br> 回款本金:' +$("#repayPrincipalMoney"+w).text()
									 + '<br> 回款利息:' +$("#repayInterestMoney"+w).text();
								}
							}
						
						var finishReturnSize = ($("#finishReturnSize")).text();
						for (var w = 1; w < finishReturnSize; w++) {
							  var data2 = ($("#finishReturnDate"+w).text());
								if(data1==data2 ){	
									ct = ct+  '<br> 回款本金:' +$("#principalMoney"+w).text()
									 + '<br> 回款利息:' +$("#interestMoney"+w).text();
								}
							}
						...
						var agentReturnSize = ($("#agentReturnSize")).text();
						for (var w = 1; w < agentReturnSize; w++) {
							  var data2 = ($("#agentReturnDate"+w).text());
								if(data1==data2 ){	
									ct = ct+  '<br> 回款本金:' +$("#agentprincipalMoney"+w).text()
									 + '<br> 回款利息:' +$("#agentinterestMoney"+w).text();
								}
							}
						...
						detail.css(calculatePos.call(target, detail, e.currentTarget)).stop().show();
...
//						}
	...
	$(".calendar-other-month").removeAttr('onclick');
		var now = new Date();
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px solid #eee;border-top: 1px solid #eee;} '
		 ...
		 + '.fullcalendar-detail{border-radius:5px;position: absolute; z-index: 10; background-color: #5288ce; display: none; opacity: 0.8; padding: 5px; text-align: right;filter: Alpha(opacity=80);width:150px;}'
