...
/*    	$(".tzjs").before("<div class='hide' id='hhproductStatus'></div>");*/
        ...
        $('.touzinput span#yueall').click(function(){
        	...
        	if(Number(czjine)>Number(sykt)){
        		if(Number(sykt)>Number(10000)){
            		$('#jine').val(10000);
            	}else{
            		$('#jine').val(sykt);
            	}
        	}else{
        		if(Number(czjine)>Number(10000)){
            		$('#jine').val(10000);
            	}else{
            		$('#jine').val(czjine);
            	}
	            ...
	            if (czjine!=""){
	            	...
	            	if(num<100){
	            		if(sykt<100){
	            			...
	            			if(num<sykt){
	            				tanchu=false;
	            				$("#hhproductStatus").text("剩余可承接债权不足100元时，需出借所有剩余额度！");
	            		...
	            		if(num==sykt){}
		            		...
		            		if((xxx+1)*100>zhye){
		            			...
		            			$('#jine').val(100);
		            			var yqsynew = (100*yjnh/365*cjqx/100).toFixed(2);//保留两位小数（预期收益=输入的金额*年化收益率/365天*期限/100）
		            			$('#calc-zongenew').text(yqsynew);
		            		}
		            		//$('#jine').val(100);
	            			tanchu=false;
	            		...
	            		tanchu=false;
	            		$("#hhproductStatus").text("最大可投金额不能大于10000元！");
		            	}else{
	            		/*if(num!=sykt){
	            		var xxx=parseInt(num/100);            		
	            		var yyy=num%100;
	            		//var zzz=(xxx+1)*100;
	            		if(((xxx+1)*100<zhye) && ((xxx+1)*100>shengyu_tk)){
	            			var zzz=(xxx)*100;
	            			//alert(zzz);
	            		}else if(((xxx+1)*100>zhye) && ((xxx+1)*100<shengyu_tk)){
	            			var zzz=(xxx)*100;
	            		}else{
	            			var zzz=(xxx+1)*100;
