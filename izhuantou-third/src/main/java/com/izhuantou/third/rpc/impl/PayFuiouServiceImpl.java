...
public class PayFuiouServiceImpl implements PayFuiouService {
	...
	if (flag) {
		...
		logger.error("PayFuiouServiceImpl类中transferFreeze方法，划拨预冻结封装condition={}" + outMemberOID + ","
			+ inMemberOID + "," + money, e.getMessage());
		return null;
	    }
	}
	return null;
	...
	if (flag) {
		...
		logger.error("PayFuiouServiceImpl类中transferFreezeToFreeze方法，冻结到冻结接封装condition={}" + outMemberOID + ","
			+ inMemberOID + "," + money, e.getMessage());
		return null;
	    }
	}
	...
	if (flag) {
		...
		if (StringUtil.isNotEmpty(memberOID) && money != null) {
		    ...
		    logger.info(
			    "PayFuiouServiceImpl类中unFreeze方法，解冻接口封装，总耗时时间：" + (System.currentTimeMillis() - startTime));
		    return resultData;
		}
	    } catch (Exception e) {
		logger.error("PayFuiouServiceImpl类中unFreeze方法，解冻接口封装condition={}" + memberOID + "," + money,
			e.getMessage());
		return null;
	    }
	}
	...
	if (flag) {
		...
		if (StringUtil.isNotEmpty(memberOID) && money != null) {
		    ...
		    logger.info("PayFuiouServiceImpl类中freeze方法，冻结封装，总耗时时间：" + (System.currentTimeMillis() - startTime));
		    return resultData;
		}
	    } catch (Exception e) {
	...
	if (flag) {
		...
		logger.error(
			"PayFuiouServiceImpl类中queryChangeCard方法，查询银行卡更换结果condition={}" + memberOID + "," + requestID,
			e.getMessage());
		return null;
	    }
	}
