...
public class SignUtil {
     ...
     * 项目初始化
     */
    public static void initProject(String projectId, String projectSecret, String itsmApiUrl) {
     ...
     */
    public static String addPersonAccount(String name, String idNo, int personArea) {
	...
	personBean.setPersonArea(personArea);
	// 所属公司,可空
     ...
     */
    public static String addOrganizeAccount(String name, int organType, OrganRegType regType, String organCode,
	    String address, int userType, String agentName, String agentIdNo) {
	...
	// 企业名称
	organizeBean.setName(name);// 天之云信息科技有限公司为公共使用，请不要修改
	// 单位类型，0-普通企业，1-社会团体，2-事业单位，3-民办非企业单位，4-党政及国家机构，默认0
	organizeBean.setOrganType(organType);
	// 企业注册类型，NORMAL:组织机构代码号，MERGE：多证合一，传递社会信用代码号,REGCODE:企业工商注册码,默认NORMAL
	organizeBean.setRegType(regType);
	// 组织机构代码号、社会信用代码号或工商注册号
	organizeBean.setOrganCode(organCode);// 52227058XT51M4AL62为公共使用，请不要修改
	// 公司地址,可空
	organizeBean.setAddress(address);
	// 经营范围,可空
	...
	if (userType == 2) {
	    // 法定代表姓名，当注册类型为2时必填??????????????
	    organizeBean.setLegalName(agentName);
     ...
     * @throws IOException
     */
    public static FileDigestSignResult platformSignByStreamm(byte[] srcPdfFile, PosBean posBean, SignType signType) {
	// 设置文件流签署的PDF文档信息
	SignPDFStreamBean signPDFStreamBean = setSignPDFStreamBean(srcPdfFile);
	// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
	/*
	 * PosBean posBean = setXYPosBean("1",170,500); // 设置签署类型为
	 * 单页签章，坐标定位方式支持单页签章、多页签章和骑缝章 SignType signType = SignType.Single; //
	 * 设置签署印章，www.tsign.cn官网设置的默认签名sealId = 0
	 */
	Properties prop = new Properties();
	InputStream url = SignUtil.class.getClassLoader().getResourceAsStream("sign.properties");
	try {
	    prop.load(url);
	} catch (IOException e) {
	    e.printStackTrace();
	    throw new RuntimeException("读取配置文件失败sealId");
	}
	...
	/*
	 * // 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效； PosBean
	 * posBean = setKeyPosBean(key,0,0,90); // 设置签署类型为 关键字签章 SignType
	...
	// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
	/*
	 * PosBean posBean = setXYPosBean("1",120,100); // 设置签署类型为 关键字签章
