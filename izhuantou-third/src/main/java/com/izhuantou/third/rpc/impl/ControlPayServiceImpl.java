...
public class ControlPayServiceImpl implements ControlPayService {
	...
	public Map<String, Object> changeCard(String memberOID) {
		try {
			...
			if (customer != null) {
				List<PayChangeCard> changeList =
						payChangeCardMapper.findChangeByRequestAndSuccess(memberOID, "0000","0000");
			...
			logger.info("ControlPayServiceImpl类中changeCard方法参数，免登陆更换银行卡接口，总耗时时间："
					+ (System.currentTimeMillis() - startTime));
			return resultmap;

		} catch (Exception e) {
			logger.error("ControlPayServiceImpl类中changeCard方法参数封装，免登陆更换银行卡接口" +memberOID, e.getMessage());
			return null;
		}
	...
	public String changeBankCardReturn(Map<String, String> map) {
		try {
			...
			logger.error("ControlPayServiceImpl类中changeBankCardReturn更换银行卡回调", e.getMessage());
			return null;
		}
	...
	public String createCustomerAccount(CustomerDTO customerDto) {
		try {
			...
			if (StringUtil.isNotEmpty(result) && "1".equals(result)) {
				...
				rowsresult = "1";
				logger.info("PayFuiouServiceImpl类中createCustomerAccount方法参数封装用户开户注册 富友，总耗时时间："
						+ (System.currentTimeMillis() - startTime));
			...
			logger.error("PayFuiouServiceImpl类中createCustomerAccount方法参数封装用户开户注册 富友", e.getMessage());
			return null;
		}
	}
	...
	public String authorizationFinish(FuyuReturnDTO result) {
		...
		try {
			...
			logger.error("PayFuiouServiceImpl类中authorizationFinish方法授权回调", e.getMessage());
			return null;
		}
	...
	public String rechargeFinish(Map<String, String> map) {
		try {
			...
			logger.error("PayFuiouServiceImpl类中rechargeFinish方法授权回调", e.getMessage());
			return null;
		}
	...
	public String withdrawalsFinish(Map<String, String> map) {
		try {
			...
			if ("0000".equals(resp_code)) {
				...
				logger.info("PayFuiouServiceImpl类中withdrawalsFinish方法,提现回调，总耗时时间："
						+ (System.currentTimeMillis() - startTime));
				return "1";
			} else {
				return FuiouErrorCode.errorCode(resp_code);
			...
			logger.error("PayFuiouServiceImpl类中withdrawalsFinish方法,提现回调", e.getMessage());
			return null;
		}
	}
