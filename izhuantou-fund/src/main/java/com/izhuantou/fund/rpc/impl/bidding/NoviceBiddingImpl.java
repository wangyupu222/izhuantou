...
public class NoviceBiddingImpl extends BaseServiceImpl<WebP2pNoviceBiddingRuning> implements NoviceBidding {
						...
						if (noviceBid.getBiddingType().equals("0")) {
							...
							if (StringUtils.isEmpty(biddto.getTqOID())) {
								...
								if (StringUtils.isNotEmpty(result) && "1".equals(result)) {
									// 将新手原标数据放到redis队列中,没有特权
									String biddtoStr = JsonUtil.toStr(biddto);
									Long mqResult = messageSender
											.put(new Message(MessageType.NOVICE_BIDDING_MESSAGE, biddtoStr));
									if (mqResult != null) {
							...
							if (StringUtils.isEmpty(biddto.getTqOID())) {
								...
								if (StringUtils.isNotEmpty(result) && "1".equals(result)) {
									// 将新手债转标数据放到redis队列中，没有特权
									String biddtoStr = JsonUtil.toStr(biddto);
									Long mqResult = messageSender
											.put(new Message(MessageType.NOVICE_TRANSFER_BIDDING_MESSAGE, biddtoStr));
									if (mqResult != null) {
										System.out.println("消息发送成功");
									} else {
										System.out.println("消息发送失败");
								...
								if (tq != null) {
									if ("0".equals(tq.getTqOID())) {
										...
										if (StringUtils.isNotEmpty(result) && "1".equals(result)) {
											...
											String biddtoStr = JsonUtil.toStr(biddto);
											Long mqResult = messageSender.put(new Message(
													MessageType.NOVICE_TRANSFER_BIDDING_MESSAGE, biddtoStr));
											if (mqResult != null) {
												System.out.println("消息发送成功");
											} else {
												System.out.println("消息发送失败");
