...
public class MobileUserController {
	...
	try {
	    ...
	    String result = userCheckSendSmsService.Send(phone_reg, name, strValidateCode);
	...
	try {
	    ...
	    if (customer != null) {
		String rows = myCashService.insertCustomer(customer);
	    ...
	    return OpResult.getFailedResult("用户信息不能为空");
	} catch (Exception e) {
	    e.printStackTrace();
	    return null;
	}
	...
	try {
		...
		Map<String, Object> map = mobileUserService.AppUpdatePayPassword(memberOID);
		if (map != null) {
