...
public class MyPrivilegeServiceImpl implements MyPrivilegeService {
	...
	try {
	    ...
	    List<MyPrivilegeDaoDTO> dto = myPrivilegeMapper.findCannotUseTQByMemberOIDIsGQ(memberOID, 1, startIndex,
		    pageSize);
	    if (dto.size() > 0) {
		for (MyPrivilegeDaoDTO daodto : dto) {
		    String psOID = daodto.getPsOID();
	...
	try {
	    ...
	    Map<String, String> prmap = ReadPropertiesl.gainPropertiesMap("privilege.properties");
