...
public class MylendServiceImpl implements MylendService {
	...
	try {
	    ...
	    /** 获取总数 */
	    int rows = this.mylendMapper.gethavingRowCount(memberOID);
		...
		MylendResultDTO lendResult = new MylendResultDTO();
		lendResult.setYsbx(mldto.getYsbx());
		lendResult.setProductStatus(mldto.getProductStatus());
		lendResult.setXmmc(mldto.getXmmc());
		lendResult.setCjje(mldto.getCjje());
		lendResult.setNhll(mldto.getNhll());
		lendResult.setTqOID(mldto.getTqOID());
		lendResult.setCashPoolOID(mldto.getCashPoolOID());
		lendResult.setOID(mldto.getOID());
		...
		if (prp != null && prp.size() > 0) {
			...
			lendResult.setCjsjTime(cjsjTime.substring(0, 10));
			// 到期时间
			Date dqsj = DateUtils.getJustDate(mldto.getDqsj().substring(0, 10));
			// 下期回款日
			String xghkr = DateUtils.formatJustDate(dqsj.getTime());
			lendResult.setXghkr(xghkr);
			// 改变后的到期时间
			Calendar cal = Calendar.getInstance();
			cal.setTime(dqsj);
			cal.add(Calendar.DAY_OF_MONTH, -1);
			String dqsjTiem = DateUtils.formatJustDate(cal.getTimeInMillis());
			lendResult.setDqsj(dqsjTiem);
		    } else {
			// 出借时间
			Date cjsj = DateUtils.getJustDate(mldto.getCjsjTime());
			String cjsjTime = DateUtils.formatJustDate(cjsj.getTime());
			lendResult.setCjsjTime(cjsjTime);
			// 剩余期数
			int syqs = mldto.getSyqs();
			String xghkr;
			try {
			    xghkr = DateUtils.gainPlusAndReduceDay(cjsjTime, 1, 0);
			    lendResult.setXghkr(xghkr);
			    lendResult.setDqsj(DateUtils.gainPlusAndReduceDay(cjsjTime, syqs, 0));
			} catch (Exception e) {
			    e.printStackTrace();
			}
		    ...
		    if (c == 0) {
			lendResult.setHkzt("正在回款中");
		    } else {
			List<Map<String, String>> Recordelist = mylendMapper
				.gainRecordeFromDebitCredit(mldto.getBusinessOID());

			if (Recordelist.size() > 0) {
			    lendResult.setHkzt("正在回款中");
			} else {
			    lendResult.setHkzt("预约中");
			}
		    }
		    // 特权
			...
			lendResult.setPrivilegeName(privilegeName);

			lendResult.setPrivilegeRange(privilegeRange);

			lendResult.setPrivilegeTerm(privilegeTerm);
			lendResult.setTqsy(tqsy);
			lendResult.setJudeg(1);
		    }
			...
			lendResult.setJXother(jx);
			ysbx = ysbx.add(payCashPool.getJXinterest());
			lendResult.setYsbx(ysbx);
		    } else {
			lendResult.setJXother(new BigDecimal(0));
		    }

		}
		resultList.add(lendResult);
	    }
